...
// форма для изменения роли для обычных пользователей (roleid = 2, Customer)
...
// обработчик изменения роли и добавление аватарки
if(isset($_POST['addadmin'])) {
    $fn = $_FILES['file']['tmp_name'];
    $file = fopen($fn, 'rb'); // открыли на по битовое чтение файла (картинки)
    $img = fread($file, filesize($fn));
    fclose($file);

    $img = addslashes($img); // проэкранируем символы в имени файла (для безопасности)
...
while($row = mysqli_fetch_array($res, MYSQLI_NUM)) {
    ...
    echo "<td>$row[1]</td>";
    echo "<td>$row[3]</td>";
    // вывод аватарки с предаварительной раскодировкой из расширения .bin
